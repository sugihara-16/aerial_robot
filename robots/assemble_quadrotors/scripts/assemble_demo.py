...
class AssembleDemo():
    def __init__(self):
        ...
        self.target_z = 0.0
        self.assemble_thre_1 = 0.55 # open state : CoG diff is 437.54 , close state : CoG diff is 420.74
        self.assemble_thre_2 = 0.44 # open state : CoG diff is 437.54 , close state : CoG diff is 420.74
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            while not rospy.is_shutdown():
                ...
                nav_msg_male.target_pos_x = -1.0
                nav_msg_male.target_pos_y = -0.5
                ...
                nav_msg_female.target_pos_x = 1.0
                nav_msg_female.target_pos_y = -0.5
            ...
            while not rospy.is_shutdown():
                ...
                if(male_female_pos[0] > self.assemble_thre_1 or abs(male_female_pos[1]) > 0.02):
                    ...
                    nav_msg_male.target_vel_x = 0.05
                    nav_msg_male.target_vel_y = 0.0
                    self.male_nav_pub.publish(nav_msg_male)
                    #female
                    nav_msg_female = FlightNav()
                    ...
                    if(not self.start_flag):
                        break
                    if(male_female_pos[0] < self.assemble_thre_2 and abs(male_female_pos[1]) < 0.02):
                        self.male_hand_pub.publish("close")
                        if(not self.close_time):
                            rospy.loginfo("close")
                            self.close_time = rospy.get_time()
                    if(self.close_time != 0  and  (rospy.get_time() - self.close_time) > 3.0):
                        rospy.loginfo("ok3")
                        #male
                        nav_msg_male = FlightNav()
                        nav_msg_male.target =1
