...
class CircTrajFollow():
  def __init__(self):
    ...
    self.radius = rospy.get_param("~radius", 0.8)
    ...
    self.nav_pub = rospy.Publisher(self.male_robot_ns+"/uav/nav", FlightNav, queue_size=1)
    self.nav_pub_female = rospy.Publisher(self.female_robot_ns+"/uav/nav", FlightNav, queue_size=1)
    self.control_sub = rospy.Subscriber(self.male_robot_ns+"/debug/pose/pid", PoseControlPid, self.controlCb)
    ...
    self.flight_nav_female = FlightNav()
  ...
  def main(self):
    ...
    while not rospy.is_shutdown():
      ...
      if self.yaw:
        ...
        self.flight_nav.target_yaw = 0
        self.flight_nav.target_omega_z = 0

      self.flight_nav_female = self.flight_nav
      self.flight_nav_female.target_yaw = 3.14 - self.flight_nav.target_yaw
      # self.flight_nav_female.target_omega_z = -self.flight_nav.target_omega_z
      self.flight_nav_female.target_omega_z = 0

      self.nav_pub.publish(self.flight_nav)
      self.nav_pub_female.publish(self.flight_nav_female)
