...
class aprilPIDcontroller():
    def __init__(self):
        ...
        #distance
        self.targetDistance = np.array([0.58171, -0.01239, 0.01017]) #x offset = 1.58171
    ...
    def defineYaw(self):
        if not self.yaw_set_flag:
            self.pre_yaw_diff = self.currentEuler[1]
            self.yaw_set_flag = True
            rospy.loginfo("set")
        yaw_chage_diff = abs(self.currentEuler[1] - self.pre_yaw_diff)
        if yaw_chage_diff > 0.05:
            rospy.loginfo("ignore unrelable data")
            return
        self.pre_yaw_diff = self.currentEuler[1]
        self.yaw = self.currentmocapEuler[2] - self.currentEuler[1]
        rospy.loginfo(self.yaw)
        if self.currentEuler[1] <= -0.1 or self.currentEuler[1] >= 0.1:
    ...
    def callback2(self, data):
        if data.pose:
            ...
            self.currentmocapEuler = np.radians(np.array([np.arctan2(2 * (moc.w * moc.x + moc.y * moc.z), 1 - 2 * (moc.x**2 + moc.y**2)),
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            # rospy.loginfo(self.currentDistance[0])
